...
// ── Measurement form import ───────────────────────────────────────────────────
// Reads body measurements from a Google Form's response tab living in the SAME
// spreadsheet, matched to a client by phone number. Header names come from the
// form's question titles, so columns are matched fuzzily by keyword.
const MEASUREMENT_KEYS = {
  ...
  thighs: ['thigh'],
  neck: ['neck'],
  bodyfat: ['body fat', 'bodyfat', 'fat'],
};
const PHONE_KEYWORDS = ['phone', 'mobile', 'contact', 'whatsapp'];

const digitsOnly = s => (s == null ? '' : String(s)).replace(/\D/g, '');

// Match two phone numbers by their trailing digits, so +91/0 prefixes still match.
function phoneMatch(a, b) {
  const da = digitsOnly(a), dbb = digitsOnly(b);
  if (da.length < 6 || dbb.length < 6) return false;
  const n = Math.min(da.length, dbb.length, 10);
  return da.slice(-n) === dbb.slice(-n);
}

export async function getLatestMeasurementForm(phone) {
  const title = (process.env.MEASUREMENT_FORM_SHEET || 'Form Responses 1').trim();
  const doc = await getDoc();
  const sheet = doc.sheetsByTitle[title];
  if (!sheet) {
    const err = new Error(
      `Form-responses tab "${title}" not found in the spreadsheet. Link your Google Form to this sheet (Form → ⋮ → Link to Sheets), or set MEASUREMENT_FORM_SHEET to the tab's exact name.`,
    );
    err.code = 'NO_FORM_TAB';
    throw err;
  }
  ...
  const findHeader = keywords =>
    headers.find(h => { const hl = h.toLowerCase(); return keywords.some(k => hl.includes(k)); });

  const phoneHeader = findHeader(PHONE_KEYWORDS);
  if (!phoneHeader) {
    const err = new Error(
      'The form-responses tab has no phone/mobile column, so responses can\'t be matched to a client. Add a "Phone" question to the Google Form.',
    );
  ...
  const measHeaders = {};
  for (const [key, kws] of Object.entries(MEASUREMENT_KEYS)) {
    const h = findHeader(kws);
    if (h) measHeaders[key] = h;
  }

  // Latest matching response = last matching row (Forms append chronologically).
  let matched = null;
  for (const r of rows) {
    if (phoneMatch(r.get(phoneHeader), phone)) matched = r;
  }
  if (!matched) return {found: false};

  const measurements = {};
  for (const [key, h] of Object.entries(measHeaders)) {
    const v = parseFloat(matched.get(h));
    if (Number.isFinite(v)) measurements[key] = v;
  }

  const tsHeader = findHeader(['timestamp']);
  return {found: true, measurements, submittedAt: tsHeader ? matched.get(tsHeader) : ''};
}
